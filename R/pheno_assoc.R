...
################## CNV-GWAS Date: April 12, 2018 Code: CNVASSOPACK002
...
prodGdsCnv <- function(phen.info, freq.cn = 0.01, snp.matrix = FALSE, n.cor = 1, 
    lo.phe = 1, chr.code.name = NULL, genotype.nodes = "CNVGenotype", coding.translate = NULL, 
    ...
    if ("CNVGenotype" %in% genotype.nodes) {
        ...
        fail <- TRUE
        trials <- 0
        ind <- seq_along(all.samples)
        while (fail && trials < 5) {
            res <- try(BiocParallel::bplapply(ind, .writeProbesCNV, all.samples = all.samples, 
                genofile = genofile, CNVsGr = CNVsGr, probes.cnv.gr = probes.cnv.gr, 
                n = n), silent = TRUE)
        ...
        ### probes.cnv.gr=probes.cnv.gr, n=n) } if(rappdirs:::get_os() == 'win'){ param <-
        ### BiocParallel::SnowParam(workers = n.cor, type = 'SOCK')
    ...
    if ("CNVgenotypeSNPlike" %in% genotype.nodes) {
        ...
        ### Define the chunks of SNPs to import chunk <- seq(from=1,
        ### to=length(probes.cnv.gr), by=chunk.len)
    ...
    ### Include LRR/BAF info if(lrr == TRUE){ importLRR_BAF(all.paths=all.paths,
    ### path.files=path.files, list.of.files=list.of.files, n.cor=1) ## TODO: Change
    ### n.cor option once parallel BUG fixed }
    ...
    SNPRelate::snpgdsClose(genofile)
    # message('GDS writen...', appendLF = FALSE) END
    # #######################################
    testit(15)
...
.prodPLINKfam <- function(all.paths) {
    ...
    famdf <- as.data.frame(cbind(FAMID, SamGen, SamGen, "NC", Sex, as.numeric(as.character(Phen[, 
        2]))))
...
.runPLINK <- function(all.paths) {
    ...
    plink <- paste0("\'", plink, "\'")
    args <- c("--gfile", mydata, "--out", plink, "--noweb")    
    ...
    file.rename( file.path(all.paths[2], "plink.gvar.summary"), 
                 file.path(all.paths[2], sumphen) )
    SNPRelate::snpgdsClose(genofile)
}
...
.prodCNVseg <- function(all.paths, probes.cnv.gr, min.sim) {
    ...
    chrx <- as.data.frame(cbind(gdsfmt::read.gdsn(gdsfmt::index.gdsn(genofile, "snp.chromosome")), 
        seq_along(gdsfmt::read.gdsn(gdsfmt::index.gdsn(genofile, 
            "snp.chromosome")))))
...
importLRR_BAF <- function(all.paths, path.files, list.of.files, n.cor = 1, verbose = FALSE) {
    (genofile <- SNPRelate::snpgdsOpen(file.path(all.paths[1], "CNV.gds"), allow.fork = TRUE, 
        readonly = FALSE))
...
.freadImport <- function(lo, list.filesLo, genofile, all.samples, nLRR = NULL, nBAF = NULL, 
    ...
    file.x <- c(as.character(list.filesLo[lo, 1]), as.character(list.filesLo[lo, 
        2]))
