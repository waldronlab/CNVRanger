...
cnvGWAS <- function(phen.info, n.cor, min.sim = 0.95, freq.cn = 0.01, snp.matrix = FALSE, 
    ...
    pdf(file.path(all.paths[3], paste0(unique(values(segs.pvalue.gr)$Phenotype), 
        "QQ-PLOT.pdf")))
    qq.plot.pdf <- qqunif.plot(values(segs.pvalue.gr)$MinPvalue, auto.key = list(corner = c(0.95, 
        0.05)))
    print(qq.plot.pdf)
    dev.off()
...
setupCnvGWAS <- function(name, phen.loc, cnv.out.loc, map.loc = NULL, folder = NULL, 
    ...
    if (is.null(map.loc)) {
        ...
        plGr <- makeGRangesFromDataFrame(probe.like.map, start.field = "V2", end.field = "V2", 
            seqnames.field = "V1")
...
#' 
#'   @export
prodGdsCnv <- function(phen.info, freq.cn = 0.01, snp.matrix = FALSE, n.cor = 1, 
    lo.phe = 1, chr.code.name = NULL, genotype.nodes = "CNVGenotype", chunk.len = 1e+06, 
    ...
    phenotypesSamX <- phenotypesSam[, c(1, (lo.phe + 1))]
    # phenotypesSamX <- na.omit(phenotypesSamX) ### Exclude samples without
    # phenotypes. i.e NA
    ...
    SNPRelate::snpgdsCreateGeno(file.path(all.paths[1], "CNV.gds"), genmat = CNVBiMa, 
        ...
        snp.rs.id = as.character(values(probes.cnv.gr)$Name), snp.chromosome = as.character(seqnames(probes.cnv.gr)), 
    ...
    if ("CNVGenotype" %in% genotype.nodes) {
        ...
        if (rappdirs:::get_os() == "unix" | rappdirs:::get_os() == "mac") {
            ...
            BiocParallel::bplapply(1:length(all.samples), .writeProbesCNV, BPPARAM = multicoreParam, 
...
}  ## BUG - small value in chuck mixing genotypes



# HELPER - Create the folder tree to keep necessary files during and after the
# analysis @param name String with a project code or name @param folder Choose
# manually the project folder (i.e. path as the root folder) @return List with
...
# HELPER - Download and test PLINK 1.07 @import utils @import rappdirs @param
# all.paths Object returned from \code{CreateFolderTree} function with the
# working folder tree @return A message of success (TRUE) of fail (FALSE) in
# running PLINK
...
# name of the populations, if using more than one @return List phen.info with
# samplesPhen, phenotypes, phenotypesdf, phenotypesSam, FamID and SexIds
# @examples phen.info <- loadPhen('/home/.../Phen.txt') sapply(phen.info, class)
.loadPhen <- function(file.nam, pheno.path, pops.names = NULL) {
    ...
    for (npop in seq_len(length(file.nam))) {
        ...
        if (all(names(dfN) == "INEXISTENT")) {
            ...
            # phenotypesdf <- as.data.frame(phenotypesdf[!duplicated(phenotypesdf),,drop =
            # FALSE])
...
# analyzed from PennCNV, general format or sequencing
...
.writeProbesCNV <- function(lo, all.samples, genofile, CNVsGr, probes.cnv.gr, n) {
    ...
    CNVSamByState <- split(CNVsGr[values(CNVsGr)$V5 == all.samples[lo]], as.character(elementMetadata(CNVsGr[values(CNVsGr)$V5 == 
        all.samples[lo]])$V4))
    for (slo in seq_len(length(CNVSamByState))) {
        state <- unique(substr(x = as.character(values(CNVSamByState[[slo]])$V4), 
            start = nchar(as.character(values(CNVSamByState[[slo]])$V4)), stop = nchar(as.character(values(CNVSamByState[[slo]])$V4))))
...
.replaceSNPtoCNV <- function(lo, all.samples, genofile, CNVsGr, probes.cnv.gr, n) {
    ...
    # 2n
    # g[c(-(values(probes.cnv.gr.0n)$tag.snp),-(values(probes.cnv.gr.1n)$tag.snp),-(values(probes.cnv.gr.3n)$tag.snp),-(values(probes.cnv.gr.4n)$tag.snp))]
    # <- '1'
...
.recodeCNVgenotype <- function(lo, genofile, all.samples, n, chunk, coding.translate) {
    ...
    #### Recode genotypes
    indexLoss <- apply(genos[, 1:2], 1, sum) > 0 & apply(genos[, 4:5], 1, sum) == 
        0
    indexGain <- apply(genos[, 1:2], 1, sum) == 0 & apply(genos[, 4:5], 1, sum) > 
        0
    indexExclude <- apply(genos[, 1:2], 1, sum) > 0 & apply(genos[, 4:5], 1, sum) > 
        0
...
.prodPLINKmap <- function(all.paths) {
    ...
    Pmap <- as.data.frame(cbind(gdsfmt::read.gdsn(gdsfmt::index.gdsn(genofile, "snp.chromosome")), 
        gdsfmt::read.gdsn(gdsfmt::index.gdsn(genofile, "snp.rs.id")), 0, gdsfmt::read.gdsn(gdsfmt::index.gdsn(genofile, 
            "snp.position"))))
    
    colnames(Pmap) <- c("Chr", "NAME", "GD", "Position")
    write.table(Pmap, file.path(all.paths[2], "mydata.map"), sep = "\t", col.names = TRUE, 
...
.prodPLINKgvar <- function(all.paths, n.cor, snp.matrix, lrr = FALSE) {
    ...
    write.table(gentype.all1, file.path(all.paths[2], "mydata.gvar"), sep = "\t", 
...
.prodPLINKfam <- function(all.paths) {
    ...
    (genofile <- SNPRelate::snpgdsOpen(file.path(all.paths[1], "CNV.gds"), allow.fork = T, 
        readonly = FALSE))  ## read GDS
    ...
    write.table(famdf, file.path(all.paths[2], "mydata.fam"), sep = "\t", col.names = TRUE, 
        row.names = FALSE, quote = FALSE)
    SNPRelate::snpgdsClose(genofile)
}
...
.runPLINK <- function(all.paths) {
    ...
    system(paste(plinkPath, "--gfile", file.path(all.paths[2], "mydata"), paste("--out", 
        file.path(all.paths[2], "plink")), "--noweb"), wait = TRUE)
    
    (genofile <- SNPRelate::snpgdsOpen(file.path(all.paths[1], "CNV.gds"), allow.fork = T, 
        readonly = FALSE))  ## read GDS
    sumphen <- paste0("plink.gvar.summary.", names(gdsfmt::read.gdsn(gdsfmt::index.gdsn(genofile, 
        "phenotype")))[[2]])
    
    file.rename(file.path(all.paths[2], "plink.gvar.summary"), file.path(all.paths[2], 
        sumphen))
    SNPRelate::snpgdsClose(genofile)
...
.prodCNVseg <- function(all.paths, probes.cnv.gr, min.sim) {
    ...
    chrx <- as.data.frame(cbind(gdsfmt::read.gdsn(gdsfmt::index.gdsn(genofile, "snp.chromosome")), 
        seq(from = 1, to = length(gdsfmt::read.gdsn(gdsfmt::index.gdsn(genofile, 
            "snp.chromosome"))), by = 1)))
...
.assoPrCNV <- function(all.paths, all.segs.gr, phenotypesSamX, method.m.test, probes.cnv.gr) {
    ...
    values(segs.pvalue.gr)$SegName <- seq(from = 1, to = length(segs.pvalue.gr), 
        by = 1)
    
    (genofile <- SNPRelate::snpgdsOpen(file.path(all.paths[1], "CNV.gds"), allow.fork = TRUE, 
        readonly = FALSE))
    
    sum.file <- paste0("plink.gvar.summary.", names(gdsfmt::read.gdsn(gdsfmt::index.gdsn(genofile, 
        "phenotype")))[[2]])
    ...
    #### Choose the method - TODO - Change if SNPMatrix implemented - Allow to get other
    #### p-values
    ...
    for (se in 1:length(segs.pvalue.gr)) {
        if (length(subsetByOverlaps(resultspGr, segs.pvalue.gr[se])) > 0) {
            ...
            names.all[[se]] <- as.character(Prbx[values(Prbx)$VALUE %in% min(values(subsetByOverlaps(resultspGr, 
...
# HELPER Extract the CNV genotypes in dosage format from GDS @param genofile is
# the loaded gds file @param snp.id is the SNP id to retrieve @param
# node.to.extract CNVgenotype or CNVgenotypeSNPlike. First one is default
...
# TODO: HELPER Create the pairwise genotypes for epistasis analysis
