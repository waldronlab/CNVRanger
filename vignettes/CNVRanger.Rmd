...
starting point for analysis with the `r Biocpkg("CNVRanger")` package.  
...
The `r Biocpkg("CNVRanger")` package uses Bioconductor core data strutures 
...
```{r rseqdata}
rseq.file <- file.path(data.dir, "all_counts_cleaned.txt")
...
## Phenotypes, pedigree and variables, if running LMM 
pedigree.loc <-  file.path(data.dir, "PhenotypePedigreeAndVariables.txt")

phen.info <- setupCnvGWAS(name="example", 
    phen.loc=phen.loc, cnv.out.loc=cnv.out.loc, map.loc=map.loc,
    pedigree.loc=pedigree.loc)
phen.info 
```
...
## Running a CNV-GWAS considering pedigree and multiple entries per sample TODO

Linear regression without can generate false positives in structured populations even when the 
estimated breeding values (EBV) are used as the response variable (**DJ PAPER**). 

```{r LMM}
## Define the model to fit
model <- formula(BT ~ Age + Year + (1|sample.id))

## Run a GWAS using a Linear Mixed Model with variables and pedigree matrix
segs.pvalue.gr <- cnvGWAS(phen.info, chr.code.name=chr.code.name, produce.gds=FALSE,
model=model, method.to.run="lmm")
```

One can use a median of LRR values instead one value per probe:
```{r LMM_lrrmean}
## Define the model to fit
model <- formula(BT ~ Age + Year + (1|sample.id))
...
## Run a GWAS using a Linear Mixed Model with variables and pedigree matrix
#segs.pvalue.gr <- cnvGWAS(phen.info, chr.code.name=chr.code.name, produce.gds=FALSE,
#model=model, method.to.run="lmm", run.lrr=TRUE, lrr.to.use="mean", model.cnv.interactions="CNVx * LRRx")
```
