...
phen.df <- read.table(phen.loc, header=TRUE, sep="\t")
colDat <- DataFrame(phen.df)
ragged.object <- RaggedExperiment(cnv.calls, colData=colDat)
phen.info <- setupCnvGWAS("example", ragged.object=ragged.object, map.loc=map.loc)
phen.info
```
