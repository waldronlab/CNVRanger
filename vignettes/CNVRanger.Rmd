...
evidence from split-reads and disconcordant read-pairs).
...
input format of `r Biocpkg("CNVRanger")`. See Section 4.1 _Input data format_ 
for details.
...
<!--
If you want to roughly calculate the absolute copy number at that position, 
you can convert out of log2 and round to the nearest integer:
...
This is not the ideal way to calculate absolute copy number, but given the 
information you have, I think it'll be about as good as you can get.
-->
...
Provided the summarized CNV regions, a valid [UCSC](https://genome.ucsc.edu/cgi-bin/hgGateway)
genome assembly, and a chromosome of interest, the function plots (from top to bottom):
