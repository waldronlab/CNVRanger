...
The function `setupCnvGWAS` imports CNV calls, phenotype information, and the probe map (if available). CNV calls can be into a (i) `GRangesList`,(ii) `r Biocpkg("RaggedExperiment")` data class as described above, (iii) PennCNV output or (iv) a general tab delimited file with five columns (chromosome, start, end, sample and copy number state). Phenotype information is a tab delimited file, such as described in [PLINK](http://zzz.bwh.harvard.edu/plink/gvar.shtml), which should contain the following columns (i) sample.id	(ii) fam (iii) sex and (iv) the last with the phenotype name (without spaces), e.g. "BT" in the vignette example that refers to breeding timing. Although family and sex are listed as columns, this info is not considered in the current implementation (i.e. can be just NA). If a map file with probe names and positions is available, it should be a tab delimited file containing three columns with the following names (i) Name (ii) Chr and (iii) Position.
...
```{r, echo=FALSE, fig.cap="Definition of CNV segments. The percentages represent the concordance between up- and downstream probes (if sequencing data, fake probe positions are automatically created based on CNV call breakpoints). These percentages are based only on CNV-samples. There is a possibility to use up- and downstream metrics, but the default considers only the percentage of concordance between the up- and downstream probes. The figure shows the default configuration. The first percentage is 100% because the single sample that was 3n is still 3n in the second probe. At some point we get 75% of concordance because 75% of 3n samples in the up- probe and also 3n in the downstream probe. Lastly, from 3 samples with 3n, only one was concordant between the up and the down probe (i.e. 33.333%). The dashed line in red shows the CNV calls used to construct the CNV segment (i.e. all 3n calls)", out.width = '100%'}
