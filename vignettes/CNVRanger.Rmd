...
package: CNVRanger
abstract: >
    The _CNVRanger_ package implements a comprehensive tool suite for the analysis of copy number variation (CNV).
...
`r Biocpkg("CNVRanger")` also provides analysis of the overlap of CNVs with
functional genomic regions such as genes, promoters, and enhancers. Finally, it
interfaces with the `r Biocpkg("edgeR")` package to provide gene expression (or other)
Quantitative Trait Loci (QTL) analysis with convenient options for common analyses 
including restriction by genomic regions and cis-QTLs.
...
For example, we annotate the steer weight and its feed conversion ratio (FCR) using simulated data.
