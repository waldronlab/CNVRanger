...
```{r cnvExprAssoc}
res <- cnvExprAssoc(cnvrs, grl, rse, window="1Mbp", verbose=TRUE)
head(res)
```
...
We first extract the CNV regions to test from the GISTIC peaks.
For demonstration, we restrict the analysis to chromosome 1 and 2. 
```{r gisticPeaks}
peaks <- granges(rowRanges(gbm[["GBM_GISTIC_Peaks-20160128"]]))
gbm <- gbm[,,-2]
genome(peaks) <- "hg19"
peaks <- peaks[seqnames(peaks) %in% c("chr1", "chr2")]
peaks
```

We accordingly also subset the CNV calls to chromosome 1 and 2. 
```{r gbmCnvCalls}
sub12 <- function(x)
{
    seqlevels(rowRanges(x)) <- paste0("chr", seqlevels(rowRanges(x)))
    genome(rowRanges(x)) <- "hg19"
    ind <- as.character(seqnames(rowRanges(x))) %in% c("chr1", "chr2")
    return(x[ind,])
}
gbm[[1]] <- sub12(gbm[[1]])
gbm[[1]]    
...
```{r tcgaGeneAnno}
library(TCGAutils)
gbm <- suppressMessages(symbolsToRanges(gbm))
gbm <- gbm[,,-3]
gbm[[2]] <- sub12(gbm[[2]])
gbm[[2]]
```

We now restrict the analysis to patients that have been measured by both assays 
...
data(sampleTypes)
sampleTypes
gbm <- suppressMessages(splitAssays(gbm, sampleCodes="01"))
```

For demonstration, we also remove instances with multiple samples for a single 
patient, and standardize sample names using the `TCGAbarcode` function 
from the `r Biocpkg("TCGAutils")` package. 

```{r gbmIntersect2}
gbm.calls <- gbm[[1]] 
gbm.rse <- gbm[[2]]
gbm.calls <- as(gbm.calls, "GRangesList")
names(gbm.calls) <- TCGAbarcode(names(gbm.calls), sample=TRUE)
colnames(gbm.rse) <- TCGAbarcode(colnames(gbm.rse), sample = TRUE)
isect <- intersect(names(gbm.calls), colnames(gbm.rse))
gbm.calls <- RaggedExperiment(gbm.calls[isect])
gbm.rse <- gbm.rse[,isect]
```

Eventually, we transform CN estimates into CN states where states are encoded as
 before by:
...
```{r transformToStates}
state <- round(mcols(gbm.calls)$Segment_Mean)
state <- vapply(state, function(i) ifelse(i < -2, -2, i), numeric(1))
state <- vapply(state, function(i) ifelse(i > 2, 2, i), numeric(1))
mcols(gbm.calls)$State <- state + 2
mcols(gbm.calls) <- mcols(gbm.calls)[,3:1]
table(mcols(gbm.calls)$State)
```
...
```{r gbmEQTL}
res <- cnvExprAssoc(peaks, gbm.calls, gbm.rse, window="1Mbp", verbose=TRUE)
res
```
