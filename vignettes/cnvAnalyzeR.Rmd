...
previously described ([da Silva et al., 2018](https://doi.org/10.1371/journal.pone.0157711)).
...
Using genomic estimated breeding values (GEBVs) for breeding time as the 
...
To function `setupCnvGWAS` imports phenotype information, CNV calls, and the probe map (if available).
...
For the example of the great tits, chromosomes _1A_, _4A_, _25LG1_, _25LG2_, and _LGE22_ 
...
probes overlapping each CNV segment is assigned as the segment _p_-value. 

This is similar to an approach used in differential expression analysis of 
...
(which is exported here to a pdf file). 
...
For convenience, a GDS file can be producedbefore the GWAS analysis with the 
`prodGdsCnv` function.
This will additionally construct a `GRanges` object containing the genomic 
...
Only the `segs.pvalue.gr`, which encloses the GWAS results, has the character 
chromosome names.  
